...
void Config::begin() {
  if (!load()) {
    // DOMYŚLNE dla nowych pól:
    g_cfg.email_pop3_enabled    = true;
    g_cfg.pop3CheckInterval_sec = 360;
    if (g_cfg.sendFTPInterval_sec == 0) g_cfg.sendFTPInterval_sec = 3600;
    if (g_cfg.cfgSyncInterval_sec == 0) g_cfg.cfgSyncInterval_sec = 3600;
...
bool Config::save(const ConfigData& d) {
  ...
  doc["email_pop3_enabled"]   = d.email_pop3_enabled;
  doc["pop3CheckInterval_sec"]= d.pop3CheckInterval_sec;
...
static void setBool(JsonDocument& doc, const char* key, bool& out) {
  JsonVariant v = doc[key];
  if (v.is<bool>())              { out = v.as<bool>(); return; }
  if (v.is<int>())               { out = (v.as<int>()!=0); return; }
  if (v.is<const char*>())       { out = (String(v.as<const char*>()) == "1" || String(v.as<const char*>()) == "true"); return; }
}
...
bool Config::load() {
  ...
  setBool  (doc, "email_pop3_enabled", g_cfg.email_pop3_enabled);
  setUInt32(doc, "pop3CheckInterval_sec", g_cfg.pop3CheckInterval_sec);

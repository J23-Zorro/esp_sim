...
static void handleMailPage(){
  ...
  // Bieżący stan auto-POP3 (EmailAlert)
  const bool     en = EmailAlert::getPop3Enabled();
  const uint32_t iv = EmailAlert::getPop3Interval();
  const uint32_t pe = EmailAlert::getPendingEpoch();
  ...
  // ====== TEST SMTP ======
  h += "<div class='card'><h3>Test SMTP</h3>";
  h += "<form method='POST' action='/mail/test'>"
       "<div class='row'><label>Wyślij do</label><input name='to'></div>"
       "<div class='row'><label>Temat</label><input name='subj' value='Test ESP32'></div>"
       "<div class='row'><label>Treść</label><textarea name='body'>Wiadomość testowa</textarea></div>"
       "<button type='submit'>Wyślij</button></form>";
  h += "</div>";

  // ====== POP3 ACK (ręczne sprawdzenie stare, GET) ======
  h += "<div class='card'><h3>POP3 ACK (ręczne)</h3>";
  h += "<form method='GET' action='/mail/check'>"
       "<div class='row'><label>EPOCH</label><input name='epoch' placeholder='np. 1736453210' required></div>"
       "<div class='row'><label>Usuń po trafieniu</label>"
       "<input type='checkbox' name='del' value='1' checked></div>"
       "<button type='submit'>Sprawdź POP3</button></form>";
  h += "<p class='muted'>Oczekiwany temat: <code>&lt;EPOCH&gt;_OK</code>, albo temat zawiera "
       "<code>&lt;EPOCH&gt;</code> i w treści jest „OK.”</p>";
  h += "</div>";

  // ====== NOWE: Automatyczne sprawdzanie POP3 (EmailAlert) ======
  h += "<div class='card'><h3>Automatyczne sprawdzanie (POP3)</h3>";
  h += "<p>Stan: " + String(en ? "WŁĄCZONE" : "WYŁĄCZONE")
       + ", interwał: " + String((unsigned long)iv) + " s"
       + " &middot; oczekiwany EPOCH: " + String((unsigned long)pe) + "</p>";

  // włącz/wyłącz + interwał
  h += "<form method='POST' action='/mail/auto'>"
       "<div class='row'><label>Włącz auto-POP3</label>"
       "<input type='checkbox' name='en' value='1' " + String(en ? "checked" : "") + "></div>"
       "<div class='row'><label>Interwał [s] (min 30)</label>"
       "<input name='int' type='number' min='30' value='" + String((unsigned long)iv) + "'></div>"
       "<button type='submit'>Zapisz</button></form>";

  // ustaw EPOCH
  h += "<form method='POST' action='/mail/set_epoch' style='margin-top:8px'>"
       "<div class='row'><label>Ustaw oczekiwany EPOCH</label>"
       "<input name='epoch' type='number' value='" + String((unsigned long)pe) + "'></div>"
       "<button type='submit'>Ustaw</button></form>";

  // ręczne sprawdzenie teraz (POST, opcjonalny epoch)
  h += "<form method='POST' action='/mail/check_now' style='margin-top:8px'>"
       "<div class='row'><label>Sprawdź teraz (opcjonalnie EPOCH)</label>"
       "<input name='epoch' type='number' placeholder='np. 1736453210'></div>"
       "<button type='submit'>Sprawdź POP3 teraz</button></form>";

  h += "</div>";

  h += "<p><a href='/menu'>← powrót</a></p>";
...
// GET /mail/check?epoch=1736453210&del=1
static void handleMailCheck(){
  if (!auth()) { return server.requestAuthentication(); }

  // gdy brak epoch – pokaż prosty formularz
  if (!server.hasArg("epoch")) {
    String h =
      "<!doctype html><meta charset='utf-8'><title>POP3 check</title>"
      "<style>body{font-family:sans-serif;margin:20px}</style>"
      "<h1>Ręczne sprawdzenie POP3 (ACK)</h1>"
      "<form method='GET' action='/mail/check'>"
      "<label>EPOCH: <input name='epoch' required></label> "
      "<label style='margin-left:8px'>Usuń wiadomość po trafieniu (del=1): "
      "<input type='checkbox' name='del' value='1' checked></label> "
      "<button type='submit'>Sprawdź</button>"
      "</form>"
      "<p>Oczekiwany temat na skrzynce: <code>&lt;EPOCH&gt;_OK</code>. "
      "Alternatywnie: temat zawiera <code>&lt;EPOCH&gt;</code> a w treści jest „OK.”</p>"
      "<p><a href='/mail'>← powrót</a></p>";
    server.send(200, "text/html", h);
    return;
  }

  uint32_t epoch = (uint32_t) strtoul(server.arg("epoch").c_str(), nullptr, 10);
  bool deleteOnMatch = (server.hasArg("del") && server.arg("del") == "1");

  Email::Ack ack;
  bool ok = Email::pop3CheckForEpoch(epoch, ack, deleteOnMatch);

  if (!ok) {
    server.send(200, "text/plain", "No ACK");
    return;
  }
  ...
  out.reserve(512);
  out += "ACK found\n";
  out += "From: ";    out += ack.from;    out += "\n";
  out += "Subject: "; out += ack.subject; out += "\n";
  out += "Date: ";    out += String((unsigned long)ack.date); out += " (epoch)\n";
  out += deleteOnMatch ? "Message deleted.\n" : "Message kept.\n";
...
static void handleMeasurePage() {
  ...
  String pretty = qjson;
    // Spróbuj sformatować JSON, a jak się nie uda – pokaż surowy
  JsonDocument tmp;
  if (deserializeJson(tmp, qjson) == DeserializationError::Ok) {
    pretty = "";
    serializeJsonPretty(tmp, pretty);
  }
...
//
static void handleMailAuto() {
  if (!auth()) { return server.requestAuthentication(); }
  bool en = server.hasArg("en") && server.arg("en") == "1";
  uint32_t iv = 300;
  if (server.hasArg("int")) {
    iv = strtoul(server.arg("int").c_str(), nullptr, 10);
    if (iv < 30) iv = 30;
    if (iv > 24UL*3600UL) iv = 24UL*3600UL;
  }
  EmailAlert::setPop3Enabled(en);
  EmailAlert::setPop3Interval(iv);
  server.sendHeader("Location", "/mail");
  server.send(302, "text/plain", "saved");
}

static void handleMailSetEpoch() {
  if (!auth()) { return server.requestAuthentication(); }
  if (!server.hasArg("epoch")) {
    server.send(400, "text/plain", "missing 'epoch'");
    return;
  }
  uint32_t ep = strtoul(server.arg("epoch").c_str(), nullptr, 10);
  EmailAlert::setPendingEpoch(ep);
  server.sendHeader("Location", "/mail");
  server.send(302, "text/plain", "ok");
}

static void handleMailCheckNow() {
  if (!auth()) { return server.requestAuthentication(); }

  // jeśli podano epoch w formularzu – użyj; w przeciwnym razie bierz z EmailAlert
  uint32_t ep = 0;
  if (server.hasArg("epoch")) {
    ep = strtoul(server.arg("epoch").c_str(), nullptr, 10);
  } else {
    ep = EmailAlert::getPendingEpoch();
  }
  if (ep == 0) {
    server.send(400, "text/plain", "Brak EPOCH (podaj w formularzu lub ustaw w EmailAlert).");
    return;
  }

  Email::Ack ack;
  bool ok = Email::pop3CheckForEpoch(ep, ack, /*deleteOnMatch=*/false);

  if (ok) {
    String msg = "ACK znaleziony dla EPOCH=" + String((unsigned long)ep) +
                 "\nSubject: " + ack.subject + "\nFrom: " + ack.from;
    server.send(200, "text/plain", msg);
  } else {
    server.send(200, "text/plain", "Brak ACK dla EPOCH=" + String((unsigned long)ep));
  }
}
...
void WebUI::begin() {
  ...
  server.on("/mail/auto",      HTTP_POST, handleMailAuto);
  server.on("/mail/set_epoch", HTTP_POST, handleMailSetEpoch);
  server.on("/mail/check_now", HTTP_POST, handleMailCheckNow);
